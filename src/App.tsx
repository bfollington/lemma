...
function CellWrapper({ children, selected, cursor, zoom }: CellWrapperProps) {
  const classes =
    [
      'cell-wrapper',
      selected && 'cell-wrapper--selected',
      cursor && 'cell-wrapper--cursor',
    ]
      .filter((s) => s)
      .reduce((s, acc) => acc + ' ' + String(s)) || ''
  return (
    <td className={classes} style={{ width: `${zoom}px`, height: `${zoom}px` }}>
      {children}
    </td>
  )
}
...
function App() {
  ...
  return (
    <div className="App">
      <button onClick={() => setZoom(zoom + 8)}>+</button>
      {zoom}
      <button onClick={() => setZoom(zoom - 8)}>-</button>
      {mode}
      <table>
            ...
            <tr key={ir}>
              ...
              {cols.map((_, ic) => {
                ...
                const selected =
                  mode === 'select' &&
                  !!selection &&
                  rectContains(selection, toPath(ic, ir))
                ...
                return (
                  <CellWrapper
                    ...
                    key={ic}
                  >
                    {cell && renderCell(cell, ic, ir, zoom)}
                  </CellWrapper>
                )
              })}
            </tr>
